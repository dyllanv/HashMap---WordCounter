...
class HashMap:
    ...
    def get(self, key):
        ...
        if self._buckets[index].contains(key):
            ...
            for i in range(self._buckets[index].size):
                if current.key == key:
    ...
    def resize_table(self, capacity):
        ...
        # Rehash key-value pairs from old hash table
        for x in range(old_capacity):
            if old_hash[x].head:
                current = old_hash[x].head
                for i in range(old_hash[x].size):
